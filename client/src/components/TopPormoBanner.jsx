...
import { PromoFloatingButton } from "./promo/PromoFloatingButton";

const REQUIRED_FIELDS = ["title", "price", "image", "description", "category"];
const MOBILE_STATE_KEY = "promoWidgetState"; // "modal","bubble","hidden"
...
export const TopPromoBanner = () => {
  ...
  const [mobileState, setMobileState] = useState(() => {
    try {
      return sessionStorage.getItem(MOBILE_STATE_KEY) || "modal";
    } catch {
      return "modal";
    }
  });
  ...
  const updateMobileState = (next) => {
    setMobileState(next);
    try {
      sessionStorage.setItem(MOBILE_STATE_KEY, next);
    } catch {
      // ignore
    }
  };
  ...
  return (
    <>
      <div className="hidden lg:flex bg-[#F1DC67] lg:min-h-25 items-center w-full overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-3 items-center justify-between gap-6">
          <div className="text-left">
            <h2 className="text-3xl xl:text-4xl font-display font-black text-black leading-tight">
              Mua sắm ngay
            </h2>
            <p className="text-sm xl:text-[15px] text-black mt-1">
              Giảm giá cho đơn hàng đầu tiên
            </p>
          </div>

          <div className="flex items-center justify-center gap-3 xl:gap-4 w-full">
          ...
          <div className="flex items-center justify-end shrink-0">
            <Link
              ...
              className="bg-green hover:bg-green-light/90 text-white text-[13px] font-bold uppercase tracking-wider px-8 py-3.5 rounded-sm transition shrink-0 whitespace-nowrap shadow-sm active:scale-95"
            >
              Mua ngay
            </Link>
          </div>
      ...
      {promoProduct && mobileState === "modal" && (
        <PromoModal
          promoProduct={promoProduct}
          onClose={() => updateMobileState("bubble")}
        />
      )}
      {promoProduct && mobileState === "bubble" && (
        <PromoFloatingButton
          promoProduct={promoProduct}
          onClose={() => updateMobileState("hidden")}
        />
      )}
    </>
  );
