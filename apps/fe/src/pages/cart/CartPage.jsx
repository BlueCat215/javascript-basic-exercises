...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    setSelectedIds((prev) => {
      ...
      cartItems.forEach((i) => {
        if (!knownIdsRef.current.has(i.productId)) next.add(i.productId);
      });
      [...next].forEach((id) => {
          ...
          <section className="lg:col-span-8 space-y-4">
            <div className="flex items-center gap-2.5 px-1">
              <input
                ...
                className="w-4.5 h-4.5 accent-green cursor-pointer"
              />
            ...
            {items.map((item) => {
              ...
              return (
                <article
                  ...
                  className={`bg-white rounded-lg border p-5 shadow-sm flex flex-col sm:flex-row gap-5 items-center transition cursor-pointer ${
                    isSelected
                      ? "border-green/50"
                ...
                >
                  <input
                    ...
                    className="w-4.5 h-4.5 accent-green cursor-pointer shrink-0 self-start sm:self-center"
                  />
                  ...
                  <div className="grow flex flex-col justify-between w-full h-full">
                    <div>
                      <h3 className="text-sm font-bold text-ink hover:text-green transition line-clamp-2">
                        {item.product?.title}
                      </h3>
                    ...
                    <div className="mt-3 flex items-center gap-1.5 text-xs font-medium text-green">
                      <CheckCircleIcon size={14} className="fill-green/20" />{" "}
                      Còn hàng
                    </div>
                  </div>
                </article>
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-24">
            <div className="bg-white rounded-lg border-2 border-green/80 p-6 shadow-sm space-y-5">
              ...
              <button
                onClick={() =>
                  navigate("/checkout", {
                    state: { voucher, selectedProductIds: [...selectedIds] },
                  })
                }
